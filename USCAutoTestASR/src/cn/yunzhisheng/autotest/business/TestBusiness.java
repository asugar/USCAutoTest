...
public class TestBusiness {
	...
	protected String getTestTask(String uri) {
		...
		try {
			...
			if (base instanceof TaskBean) {
				Constant.task = (TaskBean) base;
				result = "获取测试任务单成功";
			} else {
	...
	// 获取配置文件:get方式
	protected String getConfigFile(String uri) {
		String result = null;
		try {
			String str = mHttpClient.doGetHttpDownload(uri);
			if (str.length() == 1) {
			...
			Constant.testConfig = JsonUtil.jsonString2TestConfigBean(str);
			result = "获取测试配置文件成功";
		} catch (Exception e) {
			e.printStackTrace();
		}
